...
- `public/logo.png` is shipped **uncropped** (per explicit request) even though the source canvas has a lot of built-in padding around the mark. The nav/footer don't just scale it down (that would make the mark tiny) — `.nav-logo-mark`/`.footer-logo-mark` are fixed-size `overflow:hidden` windows containing an oversized, absolutely-positioned `.nav-logo-img`/`.footer-logo-img`, so the mark fills the header/footer via CSS clipping instead of editing the file. If the source logo is ever swapped, the img's `width`/`left`/`top` need re-tuning to re-center the new art in that window.
